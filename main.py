...
def main(argParser):
	...
	patternLength = int(eval(patternLength))
	patternLength = int(patternLength)
	if (not argParser.quiet):
		echo = "size:" + str(patternLength)
	...
	if (choice == "create"):
		patternCreated = patternCreate(patternLength, patternPath,argParser.offset, argParser.charset)
		print(patternCreated)
	elif (choice == "find"):
		origPattern = patternCreate(patternLength, patternPath,argParser.offset, charset=None)
...
def patternCreate(length: int, pathToPattern, offset=0, charset=None):
